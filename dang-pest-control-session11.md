# Dang Pest Control — Session 11 Context
...
src/components/admin/
  ...
  LeadsTab.tsx             ✅ tap-to-contact links
...
- **Edge functions deployed:**
  - `send-lead-email` — Resend integration, fires on contact + quote form submit
...
- **Resend integration** — `send-lead-email` Supabase edge function deployed; contact + quote forms fire email to `info@dangpestcontrol.com` from `leads@pestflo.ai`
...
| **Twilio** | Low | SMS opt-in on lead capture, opt-out handling, edge function |
| **Facebook token** | Low | Social posting is still demo mode — needs real FB token to go live |
| **Verify Resend live** | High | Test a real form submission and confirm email arrives at info@dangpestcontrol.com |
